 ...
 * 
 * Usa Bucket4j con Redis para rate limiting distribuido.
 * Soporta rate limiting por IP o por usuario autenticado.
 */
@Slf4j
@Aspect
@Component
...
public class RateLimitAspect {
     ...
     * 
     * Formato: "ratelimit:{prefix}:{identifier}"
     * - prefix: keyPrefix de la anotación o nombre del método
     * - identifier: IP del cliente o User ID
     */
    private String buildRateLimitKey(HttpServletRequest request, RateLimit rateLimit, ProceedingJoinPoint pjp) {
    ...
    private Bucket getBucket(String key, RateLimitType rateLimitType) {
        Supplier<BucketConfiguration> configSupplier = () -> BucketConfiguration.builder()
                .addLimit(limit -> limit
                        ...
                        .refillGreedy(rateLimitType.getCapacity(), rateLimitType.getRefillDuration()))
                .build();
     ...
     * - Retry-After: Segundos para reintentar (solo si excedido)
     */
    private void addRateLimitHeaders(ConsumptionProbe probe, RateLimitType rateLimitType) {
        ...
        long resetTimestamp = System.currentTimeMillis() / 1000 +
